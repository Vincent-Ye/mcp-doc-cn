...
提示（Prompts）使服务器能够定义可复用的提示模板和工作流，客户端可以轻松地向用户和大语言模型（LLMs）呈现这些模板和工作流。这种方式为标准化和共享常见的 LLM 交互提供了强大的支持。

<Note>
  提示的设计是 **用户可控** 的，这意味着它们从服务器暴露给客户端，用户可以明确选择使用这些提示。
</Note>
...
客户端可以通过调用 `prompts/list` 端点发现可用的提示：
  ...
  messages: [
    {
      ...
      content: {
        type: "text",
        text: "请分析以下 Python 代码并提出潜在的改进建议：\n\n```python\ndef calculate_sum(numbers):\n    total = 0\n    for num in numbers:\n        total = total + num\n    return total\n\nresult = calculate_sum([1, 2, 3, 4, 5])\nprint(result)\n```"
      }
    }
  ]
}
